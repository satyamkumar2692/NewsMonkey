...
const News = (props) => {
    ...
    const updateNews = async () => {
        try {
            ...
            let url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=7cef68035e5243c981f6ef71fadd76aa&page=${pageNo}&pageSize=${pageSize}&q=${props.q}`;
            setloading(true);
                ...
                {articles.map((element) => {
                    return <div className='col-md-3' key={element.url}>
                        <NewsItem imgUrl={element.urlToImage} title={element.title} mode={props.mode} emode={props.emode}
                            description={element.description} newsUrl={element.url} author={element.author} newsDate={element.publishedAt}
                        />
                    </div>
                }
                )}
